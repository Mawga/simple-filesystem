...
I created a disk in which the first block (0) holds directory/super block information and is loaded/saved on mount/umount

The second-fourth blocks (1-5) contain metadata of the next block for each file.
The metadata blocks contain contiguous integer values which correspond to the next block value.
...
fs_delete:
    Check for valid file_name and that it has no open fds.
